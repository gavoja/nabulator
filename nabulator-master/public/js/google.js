...
class Google {
  ...
  constructor () {
    // Get access token.
    this.token = (new URLSearchParams(location.hash)).get('access_token') || localStorage.getItem('token')
    if (this.token) {
      localStorage.setItem('token', this.token)
    }

    this.auth = { Authorization: `Bearer ${this.token}`, Accept: 'application/json' }
    location.hash = ''
  }

  async getEmail () {
    const res = await fetch(`https://www.googleapis.com/oauth2/v2/tokeninfo?accessToken=${this.token}`)
  ...
  async get () {
    ...
    const data = await (await fetch(`https://www.googleapis.com/drive/v3/files?q=${query}`, { headers: this.auth })).json()
  ...
  async load () {
    ...
    if (id) {
      const data = await (await fetch(`https://www.googleapis.com/drive/v3/files/${id}?alt=media`, { headers: this.auth })).json()
      return data
    }
  ...
  async save (data) {
    ...
    const params = {
      ...
      headers: { ...this.auth, 'Content-Type': `multipart/mixed; boundary="${BOUNDARY}"` },
